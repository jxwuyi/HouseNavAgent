...
CUDA_VISIBLE_DEVICES=0,1 python3 train.py --seed 0 --env-set small --render-gpu 1 \
    ...
    --batch-size 128 --hardness 0.95 --replay-buffer-size 400000 \
    --weight-decay 0.00001 --critic-penalty 0.0001 --entropy-penalty 0.01 \
    --batch-norm --no-debug \
    --noise-scheduler high --q-loss-coef 100 \
    --segmentation-input color --depth-input --resolution normal --history-frame-len 5 \
    --save-dir ./_model_/icml/small_object/hard_seg/ddpg_gate_hist_5 \
    --log-dir ./log/icml/small_object/hard_seg/ddpg_gate_hist_5
