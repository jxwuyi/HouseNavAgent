...
def render_episode(env, infos, stats):
    rgb = []
    segment = []
    depth = []
    eaglemap = []
    for info in infos:
        ...
        rgb.append(env.render_img('RGB'))
        segment.append(env.render_img('Segment'))
        depth.append(env.render_img('Depth'))
        eaglemap.append(env.render_img(info['loc']))
    images = dict(rgb=rgb, semantic=segment, depth=depth, eaglemap=eaglemap, target=stats['target'])
    return images
